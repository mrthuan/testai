...
public class InMobiRtbRewardedAd extends InMobiRewardedAd {
    ...
    public void internalLoadAd(InMobiInterstitialWrapper inMobiInterstitialWrapper) {
        ...
        InMobiExtras build = InMobiExtrasBuilder.build(mediationRewardedAdConfiguration.f10582d, mediationRewardedAdConfiguration.c, InMobiAdapterUtils.PROTOCOL_RTB);
        inMobiInterstitialWrapper.setExtras(build.getParameterMap());
        inMobiInterstitialWrapper.setKeywords(build.getKeywords());
        inMobiInterstitialWrapper.load(this.mediationRewardedAdConfiguration.f10580a.getBytes());
    }
}
