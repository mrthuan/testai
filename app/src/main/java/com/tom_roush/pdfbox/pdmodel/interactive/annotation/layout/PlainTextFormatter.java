...
public class PlainTextFormatter {
    ...
    private void processLines(List<PlainText.Line> list, boolean z10) {
        ...
        for (PlainText.Line line : list) {
            int i10 = AnonymousClass1.$SwitchMap$com$tom_roush$pdfbox$pdmodel$interactive$annotation$layout$PlainTextFormatter$TextAlign[this.textAlignment.ordinal()];
            if (i10 != 1) {
                if (i10 != 2) {
                    if (i10 != 3) {
                        f11 = 0.0f;
        ...
        if (plainText != null && !plainText.getParagraphs().isEmpty()) {
            ...
            for (PlainText.Paragraph paragraph : this.textContent.getParagraphs()) {
                if (this.wrapLines) {
                    ...
                    float fontSize = (this.appearanceStyle.getFontSize() * this.appearanceStyle.getFont().getStringWidth(paragraph.getText())) / 1000.0f;
                    float f10 = 0.0f;
                    if (fontSize < this.width) {
                        int i10 = AnonymousClass1.$SwitchMap$com$tom_roush$pdfbox$pdmodel$interactive$annotation$layout$PlainTextFormatter$TextAlign[this.textAlignment.ordinal()];
                        if (i10 != 1) {
                            if (i10 == 2) {
                                f10 = this.width - fontSize;
                            }
                        } else {
                            f10 = (this.width - fontSize) / 2.0f;
                        }
                    }
